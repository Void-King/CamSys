...
def move_det(camera, path, wxid):
    ...
    while(1):
        ...
        diff = cv2.threshold(diff,60,255,cv2.THRESH_BINARY)[1]#25为阈值，255为超过阈值被赋予的值
        ...
        for c in cnts:
            ...
            if cutpoints > 50:
                ...
                # strpn = path + strpn + ".jpg"
                # cv2.imwrite(strpn, frame)
                # #人脸识别
                # facedet.face_det(strpn, c, wxid)
                cutpoints = 0
            else:
                cutpoints += 1
            break
